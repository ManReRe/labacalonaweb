...
- **CI quality gates (2026-08-30):** `deploy.yml` runs an `audit` job (internal link/image check via `linkinator`, plus Lighthouse CI via `@lhci/cli`/`lighthouserc.json`) before every deploy — see the README's "Quality gates on deploy" section for what blocks vs. only warns, and why. `npm audit` flags high-severity issues in `@lhci/cli`'s transitive deps (`extract-zip`/`tmp`/`uuid`, via puppeteer/lighthouse/inquirer) — these are dev-only build tooling, never shipped to the site, and the only fix (`--force`) downgrades to an ancient `@lhci/cli@0.1.0`; leave as-is rather than "fixing" it into something worse.
